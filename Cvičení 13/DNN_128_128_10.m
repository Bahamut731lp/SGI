...
% naètení matic vah
load DNN_128_128_10_params.mat;
...
scores1 = max(scores1, 0);
%Rozšíøení vypoètené matice o vektor jednièek:
scores1 = [scores1 ones(size(scores1, 1), 1)];
assert(abs(scores1(666,111) - 7.2073) < 1e-4, "První vrstva nefunguje.", scores1(666, 111));
...
scores2 = max(scores2, 0);
%Rozšíøení vypoètené matice o vektor jednièek:
scores2 = [scores2 ones(size(scores2, 1), 1)];
% Kontrola: scores2(666,111) = 6.5808
assert(abs(scores2(666,111) - 6.5808) < 1e-4, "Druhá vrstva nefunguje.", scores2(666, 111));
